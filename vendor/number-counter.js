function NumberAnimation(e, t) {
    ...
    function n(e, t, i) {
        ...
        const t = i.number,
            ...
            p = !!i.rooms;
        s.color = s.color || "#000", s["font-size"] = s["font-size"] || "52px", e.style.display = "flex", e.style.alignItems = "center", e.style.fontSize = s["font-size"], e.style.color = "rgba(0,0,0,0)", e.textContent = t;
        const h = e.offsetHeight,
            a = e.getAttribute("data-height-space") || 2.2,
            c = i.space || h / parseFloat(a);
        e.innerHTML = "";
        let d = "";
        for (let e = 0; e < 10; e++) d += `<span style="display: block; width: ${c}px; height: ${h}px; line-height: ${h}px; ${Object.keys(s).map(e=>`${e}: inherit`).join("; ")}">${e}</span>`;
        ...
        if (p) {
            ...
            for (let e = m.length; e < r; e++) m.unshift(0)
        }
        if (i.symbol) {
            const n = [],
            ...
            const y = /(-?\d+)(\d{3})/;
            for (; y.test(a[0]);) a[0] = a[0].replace(y, "$1,$2");
            m = (a.length > 1 ? a[0] + "." + a[1] : a[0]).split("");
            for (let e = 0; e < m.length; e++) isNaN(Number(m[e])) ? "." === m[e] ? n.push(p) : n.push(r) : n.push(d);
            e.innerHTML = n.join("")
        } else {
            if (e.innerHTML = d.repeat(r), -1 !== String(t).indexOf(".")) {
            ...
            if (y) {
                ...
                t.textContent = "%", t.style.marginLeft = "2px", e.appendChild(t)
            }
        }
        ...
        for (let e = 0; e < f.length; e++) setTimeout((e, t) => {
...
window.NumberAnimation = NumberAnimation, window.initNumberAnimation = function(e, t) {
