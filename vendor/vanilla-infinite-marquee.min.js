class InfiniteMarquee {
    constructor(e = {}) {
        this.element = "string" == typeof e.element ? [...document.querySelectorAll(e.element)] : e.element, this.direction = e.direction || "left", this.spaceBetween = e.spaceBetween || "0px", this.gap = {
            vertical: e.gap && e.gap.vertical || "5px",
            horizontal: e.gap && e.gap.horizontal || "0px"
        }, this.speed = e.speed || 1e4, this.fullContainer = e.fullContainer || !1, this.fullContainerWidth = e.fullContainerWidth || 100, this.smoothEdges = e.smoothEdges || !1, this.pauseOnHover = e.pauseOnHover || !1, this.duplicateCount = e.duplicateCount || 1, this.duplicateInnerElements = e.duplicateInnerElements ? ? !0, this.breakpointSize = e.breakpointSize || 1023, this.desktopBreakpoint = this.breakpointSize + 1, this.mobileSettings = e.mobileSettings || {}, this.destroyOnDesktop = e.destroyOnDesktop || !1, this.destroyOnMobile = e.destroyOnMobile || !1, this.elementClass = e.elementClass || "marquee-container", this.on = {
    ...
    init() {
        if ("undefined" == typeof window && "undefined" == typeof document || !(Array.isArray(this.element) ? this.element.length > 0 : this.element)) this.debugging && console.error("🚊 Infinite Marquee - Failed to Initialize");
            ...
            if (this.debugging, window.matchMedia("(prefers-reduced-motion: reduce)").matches && (this.animateMotion = !1, this.debugging), "function" == typeof this.on.beforeInit && this.on.beforeInit(), Array.isArray(this.element)) {
                this.isMarqueeInitialized = !0;
                for (const e of this.element) this.configureChildNodes(e), this.configureAnimationOptions(e);
                this.debugging
            } else this.isMarqueeInitialized = !0, this.configureChildNodes(this.element), this.configureAnimationOptions(this.element), this.debugging;
    ...
    configureChildNodes(e) {
        ...
        e.classList.add(`${this.scrollType}-marquee`), e.appendChild(t), this.duplicateInnerElements && this.duplicateOriginalNodes(t), this.duplicateContainer(e), this.debugging
    }
            ...
            s = () => {
                if (this.isMarqueeInitialized) {
                    ...
                    if (e.style.setProperty("--_speed", `${a}ms`), e.style.setProperty("--_direction", r), this.smoothEdges && e.classList.add("smooth"), "vertical" === this.scrollType) {
                        ...
                        this.gap.horizontal && e.style.setProperty("--_hGap", i), this.gap.vertical && e.style.setProperty("--_vGap", s)
                    } else {
                        const i = t.matches && this.mobileSettings.spaceBetween || this.spaceBetween,
                            s = !t.matches && this.fullContainer;
                        e.style.setProperty("--_gap", i), e.classList.toggle("full", s), e.style.setProperty("--_containerWidth", s ? `${this.fullContainerWidth}%` : "")
                    }
                    window.innerWidth >= this.desktopBreakpoint && this.pauseOnHover && this.animateMotion && i(e), this.debugging && this.scrollType, this.debugging
                }
            };
    ...
    destroyOnResponsive() {
        const e = `${this.scrollType}-marquee-inner`,
            t = this.element;
        "undefined" != typeof window && (window.innerWidth <= this.breakpointSize && this.destroyOnMobile || window.innerWidth >= this.desktopBreakpoint && this.destroyOnDesktop ? this.manageMarquee(t, e) : this.isMarqueeInitialized || (this.init(), this.isMarqueeInitialized = !0))
    }
    manageMarquee(e, t) {
        this.isMarqueeInitialized && (this.destroy(e, t), this.isMarqueeInitialized = !1, this.debugging)
    }
    ...
    pause(e) {
        (e || this.element).classList.add("paused"), this.debugging, "function" == typeof this.on.pauseAnimation && this.on.pauseAnimation()
    }
    resume(e) {
        (e || this.element).classList.remove("paused"), this.debugging, "function" == typeof this.on.resumeAnimation && this.on.resumeAnimation()
    }
    debounce(e, t = 300) {
        let i;
        return (...s) => {
